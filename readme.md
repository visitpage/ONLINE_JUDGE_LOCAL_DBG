...
#endif
```
这道题目使用并查集来维护数组LR，WA了，问题出在合并数组的过程中。当需要合并三个相邻区间[l, a-1]、[a, b]和[b+1, r]时，我在合并[l, a-1]和[a, b]后，忘记再使用root更新根节点变量rt，使其指向新的根。结果我用了可能是旧的根与右侧的[b+1, r]进行合并，导致了错误。 我在使用并查集时出的错是我常常忽视正确处理根节点，因此我深感忏悔，我忏悔着地修改了并查集的代码，提醒自己总是使用并查集提供的方法来获取值。
...
|	17	|	18	| 	[19](./solved-problems/CF280C_Game_on_Tree.txt "state:unsolved")	 |	20	|	21	|	22	|	23	|
